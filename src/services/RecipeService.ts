...
export const getRecipes = async (searchFilter: SearchFilters) => {
    const url = 
    `https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=${searchFilter.categoria}&i=${searchFilter.ingrediente}`
    const {data} = await axios(url)
